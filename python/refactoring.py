...
def hashtable_itr(htable, key, flag, value):
    bucket = hashtable_get_bucket(htable, key)    
    for entry in bucket:        
        if entry[0] == key and flag == 0:          
            return entry[1]
        elif entry[0] == key and flag == 1:
            entry[1] = value           
            return
    if flag == 0:        
        return None
...
def hashtable_update(htable, key, value):
    hashtable_itr(htable, key, 1, value)

def hashtable_lookup(htable, key):
    return hashtable_itr(htable, key, 0, 0)
